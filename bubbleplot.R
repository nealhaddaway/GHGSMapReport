...
bubbleplot <- function(x_var, 
  ...
  if(is.numeric(x_var) == FALSE){
    ...
    xlabels <- labdfx$lbl #convert labels to sentence case
  }
  ...
  data <- data.frame(clr_var, 
                     ...
                     y_var, 
                     n,
                     rownum = seq(1,length(x_var)))
  data <- merge(data, 
                offset_table, 
                all.x = TRUE, 
                by.x = 'clr_var', 
                by.y = 'clr_by_name')
  data <- data[order(data$rownum),]
  ...
  x <- data %>%
    ...
    ggplot2::scale_color_brewer(palette = palette, #colour bubbles
                                #guide = FALSE,
                                name = clr_legend) +
    ggplot2::scale_size(range = c(size_from, size_to), #scale bubbles by size of n
                        #guide = FALSE,
                        name = bub_legend
                        ) + 
